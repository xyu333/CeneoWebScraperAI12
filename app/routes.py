    ...
    if request.method == "POST":
        ...
        if response.status_code == requests.codes['ok']:
            ...
            if opinions_count:
                ...
                opinions.stars = opinions.stars.apply(lambda s: round(s*MAX_SCORE))
                statistics = {
...
def charts(product_id):
    ...
    opinions['stars'] = opinions['stars'] * 5
    

    score_distribution = opinions['stars'].value_counts().reindex(np.arange(0.5, 5.5, 0.5), fill_value=0)

    fig, ax = plt.subplots()
    score_distribution.plot.bar(ax=ax)
    ...
    return render_template('charts.html', img_data=img_data, product_id=product_id)
